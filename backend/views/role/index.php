            ...
            <table class="layui-table table-hover">
                ...
                <?php $i=1;foreach($list as $vo):?>
                    <tr>
                        ...
                        <td><?=$vo->ruleName?></td>
                        <td>
                            <a href="<?=Url::to(['view','id'=>$vo->name])?>" data-opt="view" data-id="<?=$vo->name?>" class="layui-btn layui-btn-normal layui-btn-mini">权限</a>
...
<script>
    var nodes=<?=\yii\helpers\Json::htmlEncode($items)?>;
        ...
        $('a[data-opt="view"]').on('click',function(){
            ...
            $.get(url, null, function(content) {
                layer.open({
                    ...
                    success: function(layero, index){
                        layui.use(['tree_layui'],function(){
                            var tree = layui.tree_layui;
                            tree.init({
                                elem: '#demo' //传入元素选择器
                                ,nodes: nodes
                                ,check:'checkbox'
                                ,click:function(obj,node){
                                    $(obj).parent().find('>i.layui-tree-check').click()
                                }
                                ,checkclick:function(obj,node){
                                    act=$(obj).hasClass('lee-onChecked')? 'remove':'assign';
                                    saveurl=commonurl+act+'&id='+id;
                                    savedata={'routes':[node.id]};
                                    $.post(saveurl,savedata,function(res){

                                    })
                                }
                                ,activecheckid:content.data
                            },true);
                        });
                    }
                });
