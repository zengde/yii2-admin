					...
					if($this.find('dl').length > 0) {
						var $dd = $this.find('dd').each(function() {
							$(this).on('click', function() {
								...
								var href = that.config.baseUrl+$a.data('url');
	...
	function getHtml(res) {
		...
		for(var i in data) {
			...
			if(data[i].children !== undefined) {
				...
				if(data[i].data){
                    data[i].data=eval('('+data[i].data+')');
                    data[i].icon=data[i].data.icon;
				}
				...
				for(var j in data[i].children) {
					ulHtml += '<dd>';
					ulHtml += '<a href="javascript:;" data-url="' + data[i].children[j].route + '">';
                    if(data[i].children[j].data){
                        data[i].children[j].data=eval('('+data[i].children[j].data+')');
                        data[i].children[j].icon=data[i].children[j].data.icon;
					}
				...
				var dataUrl = (data[i].route !== undefined && data[i].route !== '') ? 'data-url="' + data[i].route + '"' : '';
